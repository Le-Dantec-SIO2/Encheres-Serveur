...
class EnchereRepository extends ServiceEntityRepository
{
    ...
    public function findEncheresParticipes($userId)
    {
        return $this->createQueryBuilder('e')
            ...
            ->orderBy('e.datedebut', 'ASC')
            ->select("e.id,e.leproduit_id,e.letypeenchere_id,e.datedebut,e.datefin,e.prixreserve,e.prixdepart,e.visibilite,e.lemagasin_id,e.tableau_flash,en.id,en.laenchere_id,en.leuser_id,en.prixenchere,en.dateenchere,p.id,p.nom,p.photo,p.prixreel")
    ...
    public function findEncheresFinies()
    {
        return $this->createQueryBuilder('e')
            ->innerjoin('e.leproduit', 'p')
            ->innerJoin('e.letypeenchere', 't')
            ->andWhere("e.datefin BETWEEN DATE_SUB(e.datefin,1,'month') AND CURRENT_DATE()")
            ->orderBy('e.datedebut', 'ASC')
            ->getQuery()
            ->getResult();
    }
