    ...
    public function findEncheres($enchereId = false)
    {
        ...
        if($enchereId){
            return $this->createQueryBuilder('e')
                ...
                ->select("e.id,DATE_FORMAT(e.datedebut,'%Y-%m-%d') AS date_debut,DATE_FORMAT(e.datefin,'%Y-%m-%d') AS date_fin,prixreserve,t.id AS type_enchere_id,p.id AS produit_id")            
                ->getQuery()
                ->getResult()
            ;
        }else{
