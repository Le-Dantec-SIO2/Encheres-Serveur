...
class EnchereRepository extends ServiceEntityRepository
{
     ...
     public function findEnchereTestObjet()
    {
        return $this->createQueryBuilder('e')
            ...
            ->innerJoin('en.leuser', 'u')
            ->orderBy('e.datedebut', 'ASC')
            ->select("en.prixenchere,e.id,DATE_FORMAT(e.datedebut,'%Y-%m-%d') AS date_debut,DATE_FORMAT(e.datefin,'%Y-%m-%d') AS date_fin,e.prixreserve,t.id AS type_enchere_id, p.id AS produit_id")
            ->getQuery()
            ->getResult();
            }
    /*
